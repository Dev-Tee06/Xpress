// src/Components/RiderRequests.jsx
...
export default function RiderRequests() {
  ...
  const filteredRequests = sampleRequests.filter(
    (req) =>
      req.pickup.toLowerCase().includes(search.toLowerCase()) ||
      req.delivery.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#e0f2fe] to-[#bae6fd] p-6">
      ...
      <p className="text-gray-700 mb-6">
        View all pending delivery requests. Accept jobs, check details, and
        ...
        {filteredRequests.map((req) => (
          <motion.div
            ...
            className={`bg-white shadow-lg rounded-2xl p-6 border-l-4 ${
              req.status === "Urgent" ? "border-red-500" : "border-[#0369a1]"
            }`}
          >
            ...
            <p className="text-gray-600 flex items-center gap-2">
              <FiMapPin /> Pickup: {req.pickup} | Delivery: {req.delivery}
            </p>
            <p className="text-gray-600 flex items-center gap-2">
              <FiPackage /> Package: {req.package} | Customer: {req.customer}
            </p>
            <p className="text-gray-600 flex items-center gap-2">
              <FiClock /> {req.time} | Distance: {req.distance}
            </p>
